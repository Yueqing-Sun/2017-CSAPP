...
void eval(char *cmdline) {
	 ...
	 * 阻止SIGCHLD，SIGINT和SIGTSTP信号，直到我们可以将作业添加到作业列表。 
	 */
        ...
        if (pid == 0) {
            ...
            /* Each new job must get a new process group ID 
               ...
               每个新作业必须得到一个新的进程组ID，
               以便内核不会向所有shell的作业发送ctrl-c和ctrl-z信号*/
            if (setpgid(0, 0) < 0)
...
void do_bgfg(char **argv) {
    ...
    if (isdigit(argv[1][0])) { /* 如果是数字，取的是进程号*/
        pid_t pid = atoi(argv[1]);
        if (!(jobp = getjobpid(jobs, pid))) {
            printf("(%d): No such process\n", pid);
            return;
        }
    } else if (argv[1][0] == '%') { /* 如果是%，取的是工程组号*/
    ...
    /* bg command */
    if (!strcmp(argv[0], "bg")) { /*如果是bg，恢复成后台进程 */
        ...
        /* fg command */
    else if (!strcmp(argv[0], "fg")) { /*如果是fg，恢复成前台进程 */
        if (kill(-(jobp->pid), SIGCONT) < 0)
...
/* pid2jid - Map process ID to job ID 将进程ID映射到作业ID*/
    int pid2jid(pid_t pid) {
        int i;
